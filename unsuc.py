...
insert overwrite table cc_ads.zhongyue_abnormal_board_unfinished_order_driver_1d partition(dt)

---------------------unoin all---------------------------
--select unfinsh_order_driver_d
select driver_no,driver_name,driver_phone,lease_name,second_lease_name,car_service_type,
yesterday_finish_order_num as last_cycle_finish_order_num,before_yesterday_finish_order_num as last_last_cycle_finish_order_num,1 as cycle_type,
...
) where yesterday_finish_order_num=0
union all
( --select unfinish_driver_w
select driver_no,driver_name,driver_phone,lease_name,second_lease_name,car_service_type,
last_week_finish_order_num as last_cycle_finish_order_num,last_last_week_finish_order_num as last_last_cycle_finish_order_num,2 as cycle_type,
lease_id,second_lease_no,regexp_replace(date_sub('${current_date}',1),'-','') as dt
from
(
...
where last_week_finish_order_num=0
)
union all
(--select unfinish_driver_m
select driver_no,driver_name,driver_phone,lease_name,second_lease_name,car_service_type,
last_month_finish_order_num as last_cycle_finish_order_num,last_last_month_finish_order_num as last_last_cycle_finish_order_num,3 as cycle_type,
lease_id,second_lease_no,regexp_replace(date_sub('${current_date}',1),'-','') as dt
from
(
...
where last_month_finish_order_num=0
)
